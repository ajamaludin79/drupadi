		...
		<!-- BEGIN SAMPLE TABLE PORTLET-->
			<div class="portlet box yellow">
				<div class="portlet-title">
					...
					<div class="actions">
						<a href="javascript:;" class="btn btn-default btn-sm" onclick="getModalproject();">							
				...
				<div class="portlet-body">
					...
					<div id="datatables" >
						<?php include ("list_project_ajax.php");?>  
					</div>	
				</div>				
			</div>
...
function getModalproject(){
	...
	$.ajax({
		  type: 'POST',
		  url: '<?php echo base_url('project/project/get_modal');?>',
		  data:{action:'ajax'},
		  success: function(a){
				$('#showModals').html(a);			   
...
$(document).ready(function(){			
	$(document).on('click','.tr-q', function () {	
		...
		$.ajax({
			  ...
			  success: function(a){
					...
					$('.addmodal').trigger('click'); $('.modal-title').html('Edit project');		
					$('input[name=kodeq]').val(kode);
					$('input[name=proyek]').val(that.attr('project'));					
					$('textarea[name=imgpry]').val(that.attr('imagepath'));					
  ...
  function pagination(page){
	  ...
	  var searching 	= $('.search').val();
 	  var limit  		= $('#sort').val();
	  var destination 	= '<?php echo base_url("list-proyek");?>';				  
	  $.ajax({
		  type: 'POST',
		  url: destination,
