...
class Tank_auth
{
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					if ($user->banned == 1) {									// fail - banned
						...
						$this->ci->session->set_userdata(array(
								...
								'access'		=> $user->access,
								'menu_positions' => $user->hide_menu,
								'jkelamin' 		=> $user->jkelamin,
								'first_name'	=> $user->first_name,
								'fullname'		=> $user->first_name.' '.$user->last_name,
								'status'		=> ($user->activated == 1) ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED,
						));
						$lang 		= !empty($user->language)?$user->language:'id';
						$arr_lang	= $this->get_language();
						$def_lang 	= strtolower($arr_lang[$lang]);
						
						setcookie('lang_id', $lang, time() + (86400 * 30), "/"); // 86400 = 1 day
						setcookie('language', $def_lang, time() + (86400 * 30), "/"); // 86400 = 1 day
	...
	function logout()
	{
		...
		$this->ci->session->set_userdata(array('user_id' => '', 'username' => '', 'status' => '', 'org_id' => '', 'avatar' => '', 'fullname' => '', 'access' => '', 'hide_menu' => ''));
	...
	{
		return $this->ci->session->userdata('access');
	}
	...
	function get_avatar()
	{
		$avt 		= $this->ci->session->userdata('avatar');
		$jkelamin 	= $this->ci->session->userdata('jkelamin');
		if(!empty($avt)){
			$avatar	= base_url('assets/avatar/'.$this->get_org_id().'/'.$this->get_user_id().'/'.$avt);
		}else{
			if($jkelamin=='1')
				$avatar	= base_url('assets/avatar/male_avatar.jpg');
			else	
				$avatar	= base_url('assets/avatar/female_avatar.jpg');
		}
		
		return $avatar;	
	}
	
	function each_get_avatar($userid=null,$avt=null,$jkelamin=null)
	{
		if(!empty($avt) && !empty($userid)){
			$avatar	= base_url('assets/avatar/'.$this->get_org_id().'/'.$userid.'/'.$avt);		
		}else{
			//$avatar	= base_url('assets/avatar/noavatar.png');				
			if($jkelamin=='1')
				$avatar	= base_url('assets/avatar/male_avatar.jpg');
			else	
				$avatar	= base_url('assets/avatar/female_avatar.jpg');
	...
	function create_user($username, $email, $password, $email_activation)
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
			...
			$user_id = $this->ci->users->get_ordinal($table,$field);
			$data = array(
				'id'		=> $user_id,
				'username'	=> $username,
			...
			if (!is_null($res = $this->ci->users->create_user($data, !$email_activation,$user_id))) {
    ...
    {	
		...
		$config = array(
			'upload_path'   => $path,
			'allowed_types' => 'gif|jpeg|jpg|png|tif',
			'overwrite'     => true                  
		);

        $this->ci->load->library('upload', $config);

         $images = array(); $file=''; $paths = array();
        ...
        foreach ($files['name'] as $key => $image) {
            ...
            //$fileName = preg_replace("/[^a-zA-Z0-9.]/", "", $image);//str_replace(' ', '_',$image);
            
			$ext 		= pathinfo($image, PATHINFO_EXTENSION);
			$fileName 	= time().'.'.$ext; ##image name
            ...
            if ($this->ci->upload->do_upload('images[]')) {
				...
				$paths[] = $data['full_path']; //contains full path of every image				
            } else {
                return false;
            }
        }
				
		$this->images_resize($paths);	
		
        return rtrim($file, ",");
    }
	
	##resize image
	function images_resize($path) {				
		//Image Resizing		
	...
	##date insert to DB
	function date_in_sql($mysqldate){
		if(!empty($mysqldate)){
			if($mysqldate !="0000-00-00 00:00:00" && $mysqldate !="0000-00-00"){
				$phpdate 	= strtotime( $mysqldate );
	...
	function datetime_in_sql($mysqldate){
		if(!empty($mysqldate)){
			if($mysqldate !="0000-00-00 00:00:00" && $mysqldate !="0000-00-00"){
				$phpdate 	= strtotime( $mysqldate );
				$mysqldate 	= date( 'Y-m-d H:i:s', $phpdate );
			}else{
				$mysqldate 	= "0000-00-00 00:00:00";
			}		
		}else{
			$mysqldate 	= "0000-00-00 00:00:00";
		}		
	...
	##date jam masuk kerja
	function masuk_kerja($mysqldate){
		if(!empty($mysqldate)){
			if($mysqldate !="0000-00-00 00:00:00" && $mysqldate !="0000-00-00"){
				$phpdate 	= strtotime( $mysqldate );
	...
	function get_language(){
		$lang = array('id' => 'Indonesian','is' => 'English','de' => 'Dutch','cn' => 'Chinese','fr' => 'French');			
		return $lang;		
	}
	
	##get language
	function get_kelamin(){
		$lang = array('1' => 'Laki-laki','2' => 'Perempuan');			
		return $lang;		
	}
	...
	function sum_presensi_time($times) {				
		$seconds = 0;
	...
	function calc_presensi_time($time1,$time2) {		
		$time = null;
		$notallow = array('00:00:00','--','-:-:-','');		
		if (!in_array($time1, $notallow) && !in_array($time2, $notallow)) {		
	...
	function get_presensiTime($idkary,$dates,$lok_kerja){
		##JAM Masuk					
		//$this->ci->load->model('presensi/presensi_model');	
		$jamkerja 	= $this->ci->presensi_model->get_presensi_by_id($idkary,$dates);
		$check_rest = $this->ci->presensi_model->get_worktime($lok_kerja);			
		
		$in = null;	$rest1	='--'; $rest2	='--';
		if ($jamkerja->num_rows() > 0){						
			foreach($jamkerja->result() as $jkerja){
				$in[] = $this->masuk_kerja($jkerja->date_presensi);
			}
			...
			foreach($in as $rst){
				##cek jam istirahat				
				if(isset($check_rest)){
					$rest1 	= date('H:i:s', strtotime('-2 hours', strtotime($check_rest->rest1))); 
					$rest2 	= date('H:i:s', strtotime('+2 hours', strtotime($check_rest->rest2))); 
					
					if(($rst >= $rest1 && $rst <= $rest2) && ($rst != $in[0] && $rst != end($in))) {
						$rest[]	=$rst;
					}
				}															
			} 
		...
		$jmasuk 	= ($in[0]<'12:00:00' || $jamkerja->num_rows() ==2) ? $in[0] : '--';
		$rkeluar 	= isset($rest[0]) ? $rest[0]:'--';
		$rmasuk 	= (isset($rest[0]) && ($rest[0] != end($rest))) ? end($rest) : '--';
		$jkeluar	= (end($in) >'12:00:00') ? end($in) : '--';
		
		$smasuk='--'; $tolmasuk ='--';
		if(isset($check_rest)){			
			$tolmasuk	= !empty($check_rest->toleransi_masuk) ? date('H:i:s', strtotime('-'.$check_rest->toleransi_masuk.' minutes', strtotime($jmasuk))) : '--'; //operasi pengurangan jam sebanyak 1 jam
			$masuk		= ($tolmasuk !='--') ? $this->calc_presensi_time($tolmasuk,$check_rest->jam_masuk) : $this->calc_presensi_time($jmasuk,$check_rest->jam_masuk); 
			$smasuk		= ($jmasuk !='--' && isset($check_rest)) ? $masuk : '--';
		}
		
		$spulang='--';
		if(isset($check_rest)){			
			$spulang		= ($jkeluar !='--' && isset($check_rest)) ? $this->calc_presensi_time($jkeluar,$check_rest->jam_pulang): '--';
		}
		
		$ot='--';
		if(isset($check_rest)){			
			$ot			= ($jkeluar !='--' && isset($check_rest)) ? $this->calc_presensi_time($jkeluar,$check_rest->jam_pulang) : '--';
		}
		
		$tmasuk		= ($tolmasuk !='--') ? strtotime($tolmasuk) : strtotime($jmasuk) ;
		$s_masuk	= ($smasuk !='--' && $tmasuk>strtotime($check_rest->jam_masuk)) ? $smasuk : '--';
		
		$s_pulang	= ($spulang !='--' && strtotime($jkeluar)<strtotime($check_rest->jam_pulang)) ? $spulang : '--';
		$otime		= ($ot !='--' && strtotime($jkeluar)>strtotime($check_rest->jam_pulang)) ? $ot : '--';
				
		$totresttime	= (($rkeluar !='--' && $rmasuk !='--') && isset($check_rest)) ? $this->calc_presensi_time($rmasuk,$rkeluar) : $this->calc_presensi_time($check_rest->rest2,$check_rest->rest1);
		
		##untuk total time
		//$jmsk 			= ($jmasuk 	!='--') ? $jmasuk : $check_rest->jam_masuk;
		//$jklr 			= ($jkeluar !='--') ? $jkeluar : $check_rest->jam_pulang;
		
		$jmsk 			= ($jmasuk 	!='--' && $jmasuk 	!='00:00:00') ? $jmasuk : '--';
		$jklr 			= ($jkeluar !='--' && $jkeluar	!='00:00:00') ? $jkeluar : '--';
	
		$totH			= $this->calc_presensi_time($jmsk,$jklr);		
		$tottime		= $this->calc_presensi_time($totH,$totresttime); 				
		$totHour		= $this->calc_presensi_time($jmasuk,$check_rest->jam_pulang);		
		//$n_time			= $this->calc_presensi_time($totHour,$totresttime);
		$n_time			= $this->calc_presensi_time($totHour,$jkeluar);
		
		$arr = array('in'=>$jmasuk, 'r_out'=>$rkeluar, 'r_in'=>$rmasuk, 'out'=>$jkeluar, 's_masuk'=>$s_masuk, 's_pulang'=>$s_pulang, 'n_time'=>$n_time, 'otime'=>$otime, 'tottime'=>$tottime);
		return $arr;
	}
	...
	function age_call($tgl_lahir)
	{
	   if(isset($tgl_lahir) && $tgl_lahir !="0000-00-00"){ 
			$date1 = $tgl_lahir;
			$date2 = date('Y-m-d');
